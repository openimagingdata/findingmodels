...
- **Do not use** `find_similar_models` for triage — use `findingmodel search` / triage helper only.
...
Convert only sources not skipped by triage (`exact_match`, `user_skip`). Source-first: JSON attrs from source; MD via `create_model_from_markdown`; agent fills synonyms/description in Step 5.
...
Incorporate Step 5 **metadata_proposals** into each new-model entry's Assessment.
...
## Step 8 — Read responses and fix



- `ok` or blank → no action

- Feedback → apply metadata proposals and other fixes to models; use `modify_change_from_prior.py` for CFP winnow when appropriate; re-lint
