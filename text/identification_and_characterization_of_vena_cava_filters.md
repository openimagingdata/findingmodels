# Identification and characterization of vena cava filters—`OIFM_CDE_000104`

**Synonyms:** inferior vena cava filter, IVC filter

Identification and Characterization of Vena Cava Filters

**Codes:** RADELEMENT RDES104 identification and characterization of vena cava filters; RADLEX RID480 aorta

## Attributes

### Filter count—`OIFMA_CDE_000646`

Number of filter devices   
Mininum: 0  
Maximum: 5  
Unit: unit

### Presence—`OIFMA_CDE_000649`

*(Select one)*

- **absent**  
- **present**  
- **indeterminate**  
- **unknown**  

### Location—`OIFMA_CDE_000652`

Anatomic location of filter(s) or filter components  
*(Select up to 22)*
- **infrarenal vena cava**  
- **suprarenal vena cava**  
- **right common iliac vein**  
- **left common iliac vein**  
- **right testicular vein**  
- **left testicular vein**  
- **right ovarian vein**  
- **left ovarian vein**  
- **right renal vein**  
- **left renal vein**  
- **superior vena cava**  
- **right atrium**  
- **right ventricle**  
- **left ventricle**  
- **interventricular septum**  
- **main pulmonary artery**  
- **right main pulmonary artery**  
- **left main pulmonary artery**  
- **segmental right pulmonary artery**  
- **segmental left pulmonary artery**  
- **indeterminate**  
- **unknown**  

### Complications—`OIFMA_CDE_000653`

Radiographically evident filter complications  
*(Select up to 13)*
- **absent**: none  
- **thrombosis**  
- **tilt**  
- **perforation**  
- **fracture**  
- **migration**  
- **pulmonary embolism**  
- **infection**  
- **malposition**: malposition (e.g., proximal, distal)  
- **incomplete deployment**  
- **indeterminate**  
- **unknown**  
- **other**  

---

**Contributors**

- [ACR/RSNA Common Data Elements Project](https://radelement.org/) (CDE)