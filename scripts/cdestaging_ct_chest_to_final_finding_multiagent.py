"""
Import CDEStaging CT chest definitions using the multi-agent pipeline.

This script processes Markdown and JSON definitions from the CDEStaging hood_CT_chest directory,
...
from findingmodels.cdestaging_ct_chest import should_process_file
...
def generate_attribute_report(
    ...
    lines = [
        "# CDEStaging CT Chest Definition Processing Report - Multi-Agent Pipeline",
        "",
...
async def process_cdestaging_ct_chest_directory(input_dir: str, output_dir: str, limit: Optional[int] = None):
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import CDEStaging CT chest definitions using the multi-agent pipeline"
    )
    parser.add_argument(
        "--log-level",
    ...
    await process_cdestaging_ct_chest_directory(input_dir, output_dir, limit=limit)
