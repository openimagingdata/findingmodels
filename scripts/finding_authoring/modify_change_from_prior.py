...
def main() -> None:
    ...
    for v in to_add + to_remove:
        if v not in _ADDABLE:
            print(
                f"ERROR: {v!r} not a valid direction-of-change value. "
                f"Valid: {', '.join(sorted(_ADDABLE))}",
                file=sys.stderr,
            )
            sys.exit(1)
    ...
    if cfp is None:
        print(
            "ERROR: model has no 'change from prior' attribute. "
            "Use add_change_from_prior.py to add it first.",
            file=sys.stderr,
        )
        sys.exit(1)
