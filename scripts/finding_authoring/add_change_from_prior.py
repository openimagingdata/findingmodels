...
def main() -> None:
    ...
    if not existing_oifm:
        print("ERROR: model has no oifm_id; use create_model.py for new models.", file=sys.stderr)
        sys.exit(1)

    m = re.match(r"^OIFM_([A-Z0-9]+)_\d+$", existing_oifm)
    if not m:
        print(f"ERROR: cannot parse source from oifm_id: {existing_oifm}", file=sys.stderr)
        sys.exit(1)
    source = m.group(1)

    for attr in data.get("attributes", []):
        if attr.get("name") == "change from prior":
            print("ERROR: model already has a 'change from prior' attribute", file=sys.stderr)
            sys.exit(1)
    ...
    change_attr = {
        ...
        "index_codes": [
            ...
            {"system": "SNOMED", "code": "263703002", "display": "Changed status (qualifier value)"},
        ],
    }
