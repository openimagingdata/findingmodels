...
- [ ] `description` present, ≥ 5 characters, clinically appropriate, no placeholder text (`Description for …`, `None`, `N/A`, `TODO`), no self-referential meta-commentary. **JSON-derived models with placeholder descriptions are issues to fix** — propose a real description in `metadata_proposals`. _(`metadata_fill.md`)_
- [ ] **Synonyms:** JSON-derived models with no synonyms (or only source-echo terms) need agent-proposed synonyms per `synonym_rules.md`. MD-derived models need synonym audit vs `create_model_from_markdown` output. _(`metadata_fill.md`, `synonym_rules.md`)_
...
- **metadata_proposals** — per `metadata_fill.md`: synonyms to add/remove, description if placeholder, tag tweaks, `cfp_remove` / `cfp_add` / `cfp_pairs_keep`, optional `name_change`.
...
- Additional domain attributes beyond the source are optional enrichment — note useful suggestions, but don't treat missing optional attrs as errors unless they contradict the source.
