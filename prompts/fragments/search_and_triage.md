...
Or run the chunk helper:

```bash
uv run --env-file .env python scripts/triage_cdestaging_sources.py --offset N --limit M --output reviews/triage_cdestaging_chunk_<N>.json
```
...
- **Exact match found** — state the outcome: "This already exists as `<name>` (`<oifm_id>`). Would you like to add a synonym or view it?" No triage question.
- **Only non-matches found** — state the outcome: "Searched `<q1>`, `<q2>`, `<q3>` — no existing model for this finding. Shall I create one?" No candidate list dumped on the user unless they ask.
...
The function returns the snake-cased stem; the file is at `defs/<stem>.fm.json`. Matched models may live anywhere under `defs/`, not only `defs/from_cdestaging_ct_chest/`. Confirm with `ls defs/<stem>.fm.json` before editing.

## CDEStaging batch triage

- Run triage **before** convert for each chunk; write `reviews/triage_cdestaging_chunk_<N>.json`.
